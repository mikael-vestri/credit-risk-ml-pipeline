...
def main():
    ...
    # Set up paths
    train_data_path = (
        project_root / dataset_config.PROCESSED_DATA_PATH / "engineered_features_train.parquet"
    )
    ...
    save_model(
        lr_model, "logistic_regression", models_dir, feature_names=X_train.columns.tolist()
    )
    ...
    # Save Random Forest
    save_model(
        rf_model, "random_forest", models_dir, feature_names=X_train.columns.tolist()
    )
    ...
    # Save XGBoost
    save_model(
        xgb_model, "xgboost", models_dir, feature_names=X_train.columns.tolist()
    )
