...
def validate_dataset_header_only(file_path: Path) -> dict:
    ...
    # Read header only
    df_header = pd.read_csv(file_path, nrows=0, compression=compression, low_memory=False)
    ...
    print("   [HEADER] Validation type: EXACT (header-only)")
...
def identify_leakage_columns(column_names: list[str]) -> dict[str, list[str]]:
    ...
    print("   [HEADER] Validation type: EXACT (header-based)")

    return leakage_report


def validate_dataset_sample(
    file_path: Path, sample_size: int = VALIDATION_CONFIG["sample_size"]
) -> dict:
    """
    ...
    # Read sample
    df_sample = pd.read_csv(file_path, nrows=sample_size, compression=compression, low_memory=False)
    ...
    sample_info["missing_values"] = {col: float(pct) for col, pct in missing_pct.head(20).items()}
    sample_info["high_missing_columns"] = [col for col, pct in missing_pct.items() if pct > 50]

    print(
        f"   [SAMPLE: {sample_size} rows] Columns with >50% missing: {len(sample_info['high_missing_columns'])}"
    )
    ...
    if dataset_config.TARGET_COLUMN in df_sample.columns:
        ...
        sample_info["target_distribution"] = {
            str(k): {"count": int(v), "percentage": float(v / len(df_sample) * 100)}
        ...
        ongoing_count = df_sample[
            df_sample[dataset_config.TARGET_COLUMN].isin(ongoing_statuses)
        ].shape[0]
        ...
        print(
            f"   [SAMPLE: {sample_size} rows] Ongoing loans: {ongoing_count:,} ({ongoing_count/len(df_sample)*100:.2f}%) [APPROXIMATE]"
        )
    else:
        print(
            f"   [SAMPLE: {sample_size} rows] [WARNING] Target column '{dataset_config.TARGET_COLUMN}' not found!"
        )
        sample_info["target_distribution"] = {}
    ...
    if dataset_config.DATE_COLUMN in df_sample.columns:
        ...
        if len(date_sorted) > 0:
            ...
            print(
                f"   [SAMPLE: {sample_size} rows] Date range: {date_series.min()} to {date_series.max()} [APPROXIMATE]"
            )
            print(
                f"   [SAMPLE: {sample_size} rows] Missing dates: {date_series.isna().sum():,} ({date_series.isna().sum()/len(df_sample)*100:.2f}%) [APPROXIMATE]"
            )
            print(
                f"   [SAMPLE: {sample_size} rows] Suggested split date: {suggested_split.strftime('%Y-%m-%d')} [APPROXIMATE]"
            )
            print(
                f"   [SAMPLE: {sample_size} rows] Train: {train_count:,} ({train_count/len(df_sample)*100:.2f}%) [APPROXIMATE]"
            )
            print(
                f"   [SAMPLE: {sample_size} rows] Test: {test_count:,} ({test_count/len(df_sample)*100:.2f}%) [APPROXIMATE]"
            )
    else:
        print(
            f"   [SAMPLE: {sample_size} rows] [WARNING] Date column '{dataset_config.DATE_COLUMN}' not found!"
        )
        sample_info["date_analysis"] = {}
...
def generate_validation_report(
    header_info: dict, leakage_report: dict, sample_info: dict, file_path: Path
) -> dict:
    """
...
def main():
    ...
    print(
        f"  [SAMPLE: {VALIDATION_CONFIG['sample_size']} rows] - Sample-based validation (approximate)"
    )
    print("=" * 80)
    ...
    final_report = generate_validation_report(header_info, leakage_report, sample_info, file_path)
    ...
    print(
        f"Columns to drop (high missing): {len(sample_info.get('high_missing_columns', []))} [APPROXIMATE]"
    )
    print(f"Remaining columns: {final_report['summary']['remaining_columns']}")
    print("\nValidation Types:")
    print("  [EXACT] - Schema, column names, leakage identification")
    print(
        f"  [APPROXIMATE] - Missing values, distributions (based on {VALIDATION_CONFIG['sample_size']:,} row sample)"
    )
    print("=" * 80)
