...
def load_columns_to_drop(validation_report_path: Optional[Path] = None) -> Dict[str, List[str]]:
    ...
    if validation_report_path is None:
        ...
        validation_report_path = (
            Path(__file__).parent.parent.parent / "docs" / "validation_report.json"
        )
...
def drop_columns(
    df: pd.DataFrame, columns_to_drop: List[str], reason: str = "unspecified"
...
def handle_missing_values(
    ...
    log_entry["missing_before"] = {
        ...
        for col, count, pct in zip(
            missing_before.index, missing_before.values, missing_before_pct.values
        )
        if count > 0
    }
        ...
        for col in numeric_cols:
            if df_cleaned[col].isnull().sum() > 0:
                if numeric_fill == "median":
                    ...
                    log_entry["transformations_applied"].append(
                        {"column": col, "type": "numeric", "method": "zero"}
                    )
        ...
        for col in categorical_cols:
            if df_cleaned[col].isnull().sum() > 0:
                if categorical_fill == "mode":
                    ...
                    log_entry["transformations_applied"].append(
                        {"column": col, "type": "categorical", "method": "unknown"}
                    )
    ...
    log_entry["missing_after"] = {
        col: {"count": int(count), "percentage": float(pct)}
        for col, count, pct in zip(
            missing_after.index, missing_after.values, missing_after_pct.values
        )
        if count > 0
    }
    ...
    if log_entry["transformations_applied"]:
        ...
        categorical_fills = [
            t for t in log_entry["transformations_applied"] if t["type"] == "categorical"
        ]
        ...
        if numeric_fills:
            ...
            logger.info(
                f"  Numeric columns: {', '.join(f'{k}({v} cols)' for k, v in methods_used.items())}"
            )
        ...
        if categorical_fills:
            ...
            for t in categorical_fills:
                ...
                methods_used[method] = methods_used.get(method, 0) + 1
            logger.info(
                f"  Categorical columns: {', '.join(f'{k}({v} cols)' for k, v in methods_used.items())}"
            )
...
def detect_outliers(
    ...
    outlier_report = {"method": method, "columns_checked": numeric_cols, "outliers": {}}
...
def ensure_type_consistency(
    ...
    Tuple of (cleaned DataFrame, transformation log)
    """
    log_entry = {"operation": "ensure_type_consistency", "type_changes": []}
            ...
            if col in df_cleaned.columns:
                ...
                try:
                    ...
                    log_entry["type_changes"].append(
                        {"column": col, "from": original_type, "to": "datetime64[ns]"}
                    )
    ...
    if log_entry["type_changes"]:
        for change in log_entry["type_changes"]:
            ...
            logger.info(
                f"  {change['column']}: {change['from']} -> {change['to']}"
...
def clean_and_transform(
    ...
    all_columns_to_drop = (
        columns_to_drop["leakage_columns"] + columns_to_drop["high_missing_columns"]
    )

    df_cleaned, drop_log = drop_columns(
        df_cleaned, all_columns_to_drop, reason="leakage and high missing values"
    )
    ...
    df_cleaned, missing_log = handle_missing_values(
        df_cleaned, strategy="explicit", numeric_fill="median", categorical_fill="mode"
    )
    ...
    if outlier_report["outliers"]:
        ...
        sorted_outliers = sorted(
            outlier_report["outliers"].items(), key=lambda x: x[1]["count"], reverse=True
        )[:5]
    ...
    date_cols = (
        [dataset_config.DATE_COLUMN] if dataset_config.DATE_COLUMN in df_cleaned.columns else []
    )
    ...
    # Final summary
    transformation_log["final_shape"] = {
        "rows": len(df_cleaned),
