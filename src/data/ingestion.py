...
def load_raw_data(
    raw_data_path: Path, nrows: Optional[int] = None, chunksize: Optional[int] = None
) -> pd.DataFrame:
    """
    ...
    if chunksize is not None:
        ...
        return pd.read_csv(
            file_path, compression=compression, low_memory=False, chunksize=chunksize
        )
    elif nrows is not None:
        logger.info(f"Loading sample of {nrows} rows")
        df = pd.read_csv(file_path, compression=compression, low_memory=False, nrows=nrows)
...
def ingest_raw_data(
    ...
    # Log metadata
    ingestion_metadata = log_ingestion_metadata(file_metadata, schema, stats, output_metadata_path)
