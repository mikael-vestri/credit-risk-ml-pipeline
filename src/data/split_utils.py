...
def compute_split_date(
    df: pd.DataFrame, date_column: str, train_ratio: float = 0.8
) -> pd.Timestamp:
    """
...
def validate_split_date(
    df: pd.DataFrame, date_column: str, split_date: str, mode: str = "full"
    ...
    validation_report["train_percentage"] = (
        float(train_count / total_count * 100) if total_count > 0 else 0
    )
    validation_report["test_percentage"] = (
        float(test_count / total_count * 100) if total_count > 0 else 0
    )
