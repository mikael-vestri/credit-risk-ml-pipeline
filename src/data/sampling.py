...
def sample_temporally_diverse(
    ...
    for chunk_num, chunk in enumerate(
        ...
        if matching_indices:
            ...
            logger.info(f"  Chunk {chunk_num}: Range [{chunk_start}, {chunk_end}), Extracted {found_count} rows (total so far: {total_found}/{len(all_sampled_indices)})")
        ...
        if rows_read > max(all_sampled_indices) + chunk_size_read:
            logger.warning(f"  Passed max index ({max(all_sampled_indices)}), stopping. Found {sum(len(r) for r in sampled_rows)}/{len(all_sampled_indices)} rows.")
            break
