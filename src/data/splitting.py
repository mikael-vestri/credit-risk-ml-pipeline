...
def perform_temporal_split(
    ...
    try:
        ...
        raise ValueError(f"Invalid split_date format '{split_date}'. Expected 'YYYY-MM-DD' format.") from e
    ...
    logger.info(
        f"Split complete: Train={len(train_df)} records, Test={len(test_df)} records"
    )
