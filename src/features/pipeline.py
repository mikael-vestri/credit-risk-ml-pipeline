...
def select_features(
    df: pd.DataFrame, excluded_features: list[str], target_column: str = "target"
) -> pd.DataFrame:
    """
    ...
    # Always keep target
    features_to_keep = [
        col for col in df.columns if col not in excluded_features or col == target_column
    ]
...
def engineer_features_pipeline(
    ...
    df_cleaned = prepare_target(
        df_cleaned, dataset_config.TARGET_COLUMN, dataset_config.TARGET_VALUES
    )
    ...
    train_df, test_df = perform_temporal_split(
        df_cleaned, date_column=date_column, split_date=split_date
    )
