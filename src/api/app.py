...
class LoanApplication(BaseModel):
    ...
    mths_since_last_delinq: float | None = Field(
        None, ge=0, description="Months since last delinquency"
    )
    mths_since_last_record: float | None = Field(
        None, ge=0, description="Months since last public record"
    )
    ...
    revol_util: float | None = Field(
        None, ge=0, le=100, description="Revolving line utilization rate"
    )
    total_acc: int = Field(..., ge=0, description="Total number of credit accounts")
    last_fico_range_high: int | None = Field(
        None, ge=300, le=850, description="Last reported FICO range high"
    )
    last_fico_range_low: int | None = Field(
        None, ge=300, le=850, description="Last reported FICO range low"
    )
    ...
    mths_since_last_major_derog: float | None = Field(None, ge=0)
    policy_code: int = Field(1, description="Policy code")
    acc_now_delinq: int = Field(0, ge=0, description="Accounts currently delinquent")
    tot_coll_amt: float | None = Field(None, ge=0)
    tot_cur_bal: float | None = Field(None, ge=0)
    open_acc_6m: int | None = Field(None, ge=0)
    open_act_il: int | None = Field(None, ge=0)
    open_il_12m: int | None = Field(None, ge=0)
    open_il_24m: int | None = Field(None, ge=0)
    mths_since_rcnt_il: float | None = Field(None, ge=0)
    total_bal_il: float | None = Field(None, ge=0)
    il_util: float | None = Field(None, ge=0, le=100)
    open_rv_12m: int | None = Field(None, ge=0)
    open_rv_24m: int | None = Field(None, ge=0)
    max_bal_bc: float | None = Field(None, ge=0)
    all_util: float | None = Field(None, ge=0, le=100)
    total_rev_hi_lim: float | None = Field(None, ge=0)
    inq_fi: int | None = Field(None, ge=0)
    total_cu_tl: int | None = Field(None, ge=0)
    inq_last_12m: int | None = Field(None, ge=0)
    acc_open_past_24mths: int | None = Field(None, ge=0)
    avg_cur_bal: float | None = Field(None, ge=0)
    bc_open_to_buy: float | None = Field(None, ge=0)
    bc_util: float | None = Field(None, ge=0, le=100)
    chargeoff_within_12_mths: int = Field(0, ge=0)
    delinq_amnt: float | None = Field(None, ge=0)
    mo_sin_old_il_acct: float | None = Field(None, ge=0)
    mo_sin_old_rev_tl_op: float | None = Field(None, ge=0)
    mo_sin_rcnt_rev_tl_op: float | None = Field(None, ge=0)
    mo_sin_rcnt_tl: float | None = Field(None, ge=0)
    mort_acc: int | None = Field(None, ge=0)
    mths_since_recent_bc: float | None = Field(None, ge=0)
    mths_since_recent_bc_dlq: float | None = Field(None, ge=0)
    mths_since_recent_inq: float | None = Field(None, ge=0)
    mths_since_recent_revol_delinq: float | None = Field(None, ge=0)
    num_accts_ever_120_pd: int | None = Field(None, ge=0)
    num_actv_bc_tl: int | None = Field(None, ge=0)
    num_actv_rev_tl: int | None = Field(None, ge=0)
    num_bc_sats: int | None = Field(None, ge=0)
    num_bc_tl: int | None = Field(None, ge=0)
    num_il_tl: int | None = Field(None, ge=0)
    num_op_rev_tl: int | None = Field(None, ge=0)
    num_rev_accts: int | None = Field(None, ge=0)
    num_rev_tl_bal_gt_0: int | None = Field(None, ge=0)
    num_sats: int | None = Field(None, ge=0)
    num_tl_120dpd_2m: int | None = Field(None, ge=0)
    num_tl_30dpd: int | None = Field(None, ge=0)
    num_tl_90g_dpd_24m: int | None = Field(None, ge=0)
    num_tl_op_past_12m: int | None = Field(None, ge=0)
    pct_tl_nvr_dlq: float | None = Field(None, ge=0, le=100)
    percent_bc_gt_75: float | None = Field(None, ge=0, le=100)
    pub_rec_bankruptcies: int | None = Field(None, ge=0)
    tax_liens: int | None = Field(None, ge=0)
    tot_hi_cred_lim: float | None = Field(None, ge=0)
    total_bal_ex_mort: float | None = Field(None, ge=0)
    total_bc_limit: float | None = Field(None, ge=0)
    total_il_high_credit_limit: float | None = Field(None, ge=0)

    # Engineered features (will be computed if not provided)
    funded_to_loan_ratio: float | None = Field(None, ge=0)
    installment_to_income_ratio: float | None = Field(None, ge=0)
    fico_midpoint: float | None = Field(None, ge=300, le=850)
    fico_range: float | None = Field(None, ge=0)
    credit_history_years: float | None = Field(None, ge=0)
    issue_year: int | None = Field(None, ge=2007, le=2018)
    issue_month: int | None = Field(None, ge=1, le=12)
...
async def get_stats():
    ...
    """
    return {"request_stats": request_metrics.get_stats(), "timestamp": time.time()}
    ...
    try:
        ...
        if input_dict.get("installment_to_income_ratio") is None:
            ...
            input_dict["installment_to_income_ratio"] = (
                input_dict["installment"] / monthly_income if monthly_income > 0 else 0
            )

        if input_dict.get("fico_midpoint") is None:
            input_dict["fico_midpoint"] = (
                input_dict["fico_range_low"] + input_dict["fico_range_high"]
            ) / 2

        if input_dict.get("fico_range") is None:
            input_dict["fico_range"] = input_dict["fico_range_high"] - input_dict["fico_range_low"]
        ...
        request_metrics.record_prediction(result["prediction"], result["default_probability"])
